...
# Import torch and immediately disable dynamo
import torch

# Multiple ways to disable dynamo
torch._dynamo.config.disable = True
torch._dynamo.reset()

# # Verify it's disabled
# print(f"Dynamo disabled via env: {os.environ.get('TORCH_DYNAMO_DISABLE')}")
# print(f"Dynamo config disabled: {torch._dynamo.config.disable}")
...
def load_llava_dataset(path):         
    ext = path.split(".")[-1]
    if ext == "json":                
        return load_dataset("json", data_files=path, split="train")
    else:
        return load_dataset(path, split="train") # Load JSON dataset using HuggingFace
...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args):
        ...
        if "{" in data_path and "}" in data_path:
            ...
            for file_name in file_names:
                ...
                dataset = load_llava_dataset(full_path)
                rank0_print(f"Loaded {len(dataset)} samples from {full_path}")
                self.list_data_dict.extend(dataset.to_list())

        elif data_path.endswith(".yaml"):
            with open(data_path, "r") as file:
                yaml_data = yaml.safe_load(file)
                datasets_config = yaml_data.get("datasets", [])
                data_args.dataset_paths = [d.get("json_path") for d in datasets_config]

                for dataset_cfg in datasets_config:
                    json_path = dataset_cfg.get("json_path")
                    sampling_strategy = dataset_cfg.get("sampling_strategy", "all")
                    sampling_number = None

                    rank0_print(f"Loading {json_path} with {sampling_strategy} sampling strategy")

                    # Load using HuggingFace
                    dataset = load_llava_dataset(json_path)

                    if ":" in sampling_strategy:
                        sampling_strategy, sampling_number = sampling_strategy.split(":")
                        if "%" in sampling_number:
                            sampling_number = math.ceil(int(sampling_number.rstrip("%")) * len(dataset) / 100)
                        else:
                            sampling_number = int(sampling_number)

                    if sampling_strategy == "first" and sampling_number is not None:
                        dataset = dataset.select(range(sampling_number))
                    elif sampling_strategy == "end" and sampling_number is not None:
                        dataset = dataset.select(range(len(dataset) - sampling_number, len(dataset)))
                    elif sampling_strategy == "random" and sampling_number is not None:
                        dataset = dataset.shuffle(seed=42).select(range(sampling_number))

                    rank0_print(f"Loaded {len(dataset)} samples from {json_path}")
                    self.list_data_dict.extend(dataset.to_list())

        else:
            data_args.dataset_paths = [data_path]
            rank0_print(f"Loading {data_path}")
            dataset = load_llava_dataset(data_path)
            rank0_print(f"Loaded {len(dataset)} samples from {data_path}")
            self.list_data_dict.extend(dataset.to_list())

        rank0_print(f"Total loaded: {len(self.list_data_dict)} samples")
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            if type(self.list_data_dict[i]["image"]) is not list:
                self.list_data_dict[i]["image"] = [self.list_data_dict[i]["image"]]
            if len(self.list_data_dict[i]["image"]) > 1:                        
                image = [self.process_image(f, "pad") for f in self.list_data_dict[i]["image"]]
                image = [[im[0], im[1], "image"] for im in image]
            else:
                image = [self.process_image(f) for f in self.list_data_dict[i]["image"]]
